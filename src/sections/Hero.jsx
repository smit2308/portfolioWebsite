...
const Hero = () => {
  ...
  // Check if the Mobile Navbar menu is open
  const toggleMenu = () => {
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div className="max-container h-screen w-full  flex flex-row max-sm:flex-col items-center justify-between max-sm:justify-center  lg:pl-16 md:pl-10 pl-6 max-sm:p-0  ">
            ...
            <div className=" bg-primary rounded-lg flex flex-col   gap-6 lg:px-10 px-6 pb-10 pt-6 shadow-xl">
              ...
              {resumeLinks.map((item, index) => (
                <a
                  ...
                  onClick={toggleMenu}
                  rel="noreferrer"
                  className="font-montserrat  text-secondary max-sm:text-lg text-xl hover:text-accent hover:underline transition-all ease-in-out "
                >
                  {item.label}
                </a>
              ))}
        ...
        >
          <div className="my-auto flex flex-col gap-2 max-sm:items-center max-sm:gap-2">
            ...
            <div className="max-xl:h-[20vw] h-72 max-sm:h-32 mb-4 max-sm:w-80 ">
              <h1
                className="pb-6 h-full font-display italic  xl:text-[150px] xl:leading-[130px] max-xl:text-[12vw] max-xl:leading-[10vw] max-sm:text-[72px] max-sm:leading-[56px] 
        ...
        >
          <img
            className="sm:object-cover object-left w-full   max-lg:hidden "
            src={Myimage}
          />

          <img
            className="sm:object-cover w-full object-bottom sm:object-left lg:hidden  max-sm:object-cover   "
            src={Myimage_small}
          />
        </motion.div>
      </motion.div>
